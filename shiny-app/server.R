...
indigenous[, 
           "perc_in_state" := round(indigenous * 100 / (non_indigenous + indigenous + non_declared), 2), 
           by = year]
...
shinyServer(function(input, output, session){
  ...
  output$sb_states <- renderUI({checkboxGroupInput("sb_states", "Choose States:", 
  ...
  # --- INDIGENOUS SESSION ----------------------------------------------------------------------------------
  # Separate data based on the selected states
  indg_data_states <- reactive({indigenous[state %in% input$sb_states]})
  
  # Separete which value is to be plotted
  indg_data <-reactive({switch(input$indg_analysis,
                               "0" = {indg_data_states()[, "value" := indigenous][]},
                               "1" = {indg_data_states()[, "value" := perc_in_state][]})})
  
  # The rendered title
  output$indg_title <- renderText({switch(input$indg_analysis,
                                          "0" = {paste("Total Indigenous Population in", input$indg_myYear)},
                                          "1" = {paste("Indigenous Percentage of the State Population in ", 
                                                       input$indg_myYear, " (%)")})})

  # Select the year to plot in geomap
  indg_data_geochart <- reactive({indg_data()[year == input$indg_myYear][]})
  
  output$indg_geochart <- renderGvis({gvisGeoChart(data = indg_data_geochart(),
                                                   locationvar = "state", 
                                                   colorvar = "value",
  ...
  # Plot line of the population number along the years
  output$indg_stackchart <- renderChart2({n <- nPlot(data = indg_data(), 
                                                     value ~ year, 
                                                     group = "state", 
  ...
  pop_data_states_type <-reactive({switch(input$pop_analysis,
                                          "0" = {pop_data_states()[area == "urban"]},
                                          "1" = {pop_data_states()[area == "rural"]},
                                          "2" = {pop_data_states()[area == "both"]})})
  
  # Separate data to the geo plot
  pop_data_states_type_year <- reactive({switch(input$pop_myYear,
  ...
  output$pop_geochart <- renderGvis({gvisGeoChart(data = pop_data_states_type_year(),
                                                  locationvar = "state",
                                                  colorvar = "value",
                                                  options = list(region = "BR",
                                                                 displayMode = "regions", 
  ...
  pop_stackchart_data <- reactive({melt(pop_data_states_type(), 
                                        id.vars = "state", 
  ...
  output$pop_stackchart <- renderChart2({n <- nPlot(data = pop_stackchart_data(),
                                                    population ~ year, 
                                                    group = "state", 
                                                    type = "stackedAreaChart")
                                         n$chart(margin=list(left=100))
  ...
  pop_title_aux <- renderText({switch(input$pop_analysis,
                                      "0" = {"Urban"},
                                      "1" = {"Rural"},
                                      "2" = {"Total"})})
